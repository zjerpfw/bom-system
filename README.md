...
1. 管理员在前端「物料」页导入ERP商品/物料CSV，形成系统匹配底库。
2. OCR识别图纸、表格或接收语音/文本录入。
3. 系统用研发叫法匹配ERP物料主数据。
4. 人工审核待确认和缺失物料。
5. 导出可导入ERP的Excel文件。

技术栈：Python 3.11、FastAPI、SQLite、SQLAlchemy、PaddleOCR、百度OCR API、OpenAI兼容/阿里百炼/百度千帆 Embedding、FAISS、GPT-4o-mini、Vue 3、Vite、Vant 4、openpyxl。
...
- `EMBEDDING_PROVIDER`：向量供应商，支持 `openai`、`dashscope`、`qianfan`。
- `DASHSCOPE_API_KEY`、`DASHSCOPE_BASE_URL`、`DASHSCOPE_EMBEDDING_MODEL`：阿里百炼向量配置，默认模型 `text-embedding-v4`。
- `QIANFAN_API_KEY`、`QIANFAN_BASE_URL`、`QIANFAN_EMBEDDING_MODEL`：百度千帆向量配置，默认模型 `embedding-v1`。
...
QIANFAN_API_KEY=你的百度千帆API Key
QIANFAN_EMBEDDING_MODEL=embedding-v1
```

运行后也可以在前端「设置」页切换 `规则模式 / AI增强模式`，并维护接口地址、密钥、聊天模型、向量模型和向量供应商。数据库中的系统配置优先级高于 `.env`，密钥只保存不明文回显。

## ERP物料库维护

系统拿研发上传的原始叫法和 ERP 商品/物料底库做对比。前端入口在底部「物料」页：

1. 上传 ERP 导出的 CSV，列名必须包含 `编码`、`名称`、`规格`、`单位`、`类别`。
2. 点击「导入或更新物料库」，编码重复时会更新原记录。
3. 开启 AI 增强时，导入后点击「重建AI匹配索引」。

研发侧入口仍是底部「上传」页；上传图纸、表格、文本或语音后，系统会自动进入匹配和审核流程。
